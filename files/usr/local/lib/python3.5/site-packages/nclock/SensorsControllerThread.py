...
class SensorsControllerThread(Thread):
        ...
        while not self._stop_event.wait(SensorsControllerThread.POLL_INTERVAL):
            ...
            if self._settings.get("button.status") == "on":
                ...
                if self._button_count > 3 / SensorsControllerThread.POLL_INTERVAL:
                    self._settings.log.msg("SensorsControllerThread: button (%s)" % "long")z
    ...
    def _update_button(self, new):
        ...
        if new:
            ...
            value = "off"
        self._settings.log.msg("SensorsControllerThread: button (%s)" % value)
    ...
    def _update_sensor(self):
        ...
        if distance < self._sensor_threshold:
            value = "on"
            self._settings.log.msg(
                "SensorsControllerThread: sensor on (distance is %s)" % distance)
        else:
            value = "off"
        if old_value != value:
            self._settings.set(name, value)
