...
class SensorsControllerThread(Thread):
    ...
    def run(self):
        ...
        if self._state == "starting":
            ...
            self._expected_distance = (sum(values) - min(values) - max(values)) / (len(values) - 2)
            self._settings.log.msg(
    ...
    def _update_sensor(self):
        ...
        if distance > self._expected_distance + (self._expected_distance * self._sensor_threshold) or \
           distance < self._expected_distance - (self._expected_distance * self._sensor_threshold):
            value = "on"
            self._settings.log.msg("SensorsControllerThread: Hand detected (%s)" % distance)
        else:
            value = "off"
        if old_value != value:
            self._settings.set(name, value)
