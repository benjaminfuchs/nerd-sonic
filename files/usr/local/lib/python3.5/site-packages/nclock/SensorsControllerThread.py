...
class SensorsControllerThread(Thread):
    ...
    def __init__(self, settings):
        ...
        self._sensor_type = Button(
            int(settings.get_value('SENSOR', 'ultraschall')))
        if self._sensor_type == "SRF02":
            self._sensor = SRF02(0x70)
        elif self._sensor_type == "HCSR04":
            self._sensor = HCSR04(18, 22)
        elif self._sensor_type == "HCSR04":
                self._settings.log.msg(
                    "SensorsControllerThread: [ERROR] Unkown sensor type (%s)" % str(self.self._sensor_type))
