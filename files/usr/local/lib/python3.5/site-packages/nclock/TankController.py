...
class TankController():
    ...
    def __init__(self, settings):
        ...
        self._buzzer = Buzzer(int(settings.get_value('GPIO', 'buzzer')))
    ...
    def on_count(self, name, old, new):
        ...
        self._settings.log.msg(
            "TankController: [INFO] count (%s)" % new)
        if new * self._milliliter >= self._empty * self._size:
            self._settings.set('tank.status', 'empty')
            self._buzzer.beep()
        elif new * self._milliliter >= self._warning * self._size:
            self._settings.set('tank.status', 'warning')

    def on_button(self, name, old, new):
        """ process button changes """

        if new == "on":
            self._settings.set('tank.status', 'full')
