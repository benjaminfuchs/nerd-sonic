...
class DisplayController():
    ...
    def show_settings(self):
        ...
        self._draw_settings_string(
            0, "os      : " + " ".join(distro.linux_distribution()))
        self._draw_settings_string(1, "hostname: " + socket.gethostname())
        ip = netifaces.ifaddresses('wlan0')[netifaces.AF_INET][0]['addr']
        self._draw_settings_string(2, "ip      : " + ip)
        with open('/proc/uptime', 'r') as f:
            uptime_seconds = float(f.readline().split()[0])
        self._draw_settings_string(
            3, "uptime  : " + str(round(uptime_seconds / 60 / 60 / 24, 1)) + " days")
        self._draw()
    ...
    def _create_plot(dates, values, format_string):
        try:
            ...
            plt.savefig(os.path.join(default_store, 'plot.png'))
            DisplayController._convert_png_to_bmp(os.path.join(default_store, 'plot.png'),
                                                  os.path.join(default_store, 'plot.bmp'))
        except ValueError:
            return '/home/pi/Pictures/error.bmp'
        return os.path.join(default_store, 'plot.bmp')

    @staticmethod
    def _convert_png_to_bmp(source, target):
    ...
    def _draw_time(self):
        ...
        font = ImageFont.truetype(
            '/var/lib/nerd-sonic/fonts/ubuntu-font-family-0.83/UbuntuMono-R.ttf', font_size)
    ...
    def _draw_settings_menu(self):
        self._draw_menu_rectangle(0, None)
    ...
    def _draw_settings_string(self, num, text):
        block_size = self._menu_block_size
        font = ImageFont.truetype(
            '/var/lib/nerd-sonic/fonts/ubuntu-font-family-0.83/UbuntuMono-R.ttf', 18)
        self._epd.draw_string_at(
            self._frame_black, 2, 20 * num + block_size + 10, text, font, COLORED)
