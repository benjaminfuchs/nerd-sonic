        ...
        while not self._stop_event.wait(DisplayStateThread.POLL_INTERVAL):
            try:
                ...
                if self._state_change:
                    self._settings.log.msg(
                        "DisplayStateThread: processing new state ...")
                    self._state_change = False
                    ...
                    if self._state == "settings":
                        self._display.show_settings()

                    if "tank" in self._state:
                        if "week" in self._state:
                            self._display.draw_sub_menu(1)
                            times, values = self._settings.database.get_week(
                                "sensors.tank.value")
                            self._display.draw_plot_week(times, values)
                            self._display.show()
                        if "month" in self._state:
                            self._display.draw_sub_menu(2)
                            times, values = self._settings.database.get_week(
                                "sensors.tank.value")
                            self._display.draw_plot_month(times, values)
                            self._display.show()
                        if "year" in self._state:
                            self._display.draw_sub_menu(3)
                            times, values = self._settings.database.get_week(
                                "sensors.tank.value")
                            self._display.draw_plot_year(times, values)
                            self._display.show()
            except Exception as e:
                self._settings.log.msg("DisplayStateThread: Exception (%s)" % e.message())
