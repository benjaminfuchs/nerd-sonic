...
class DisplayStateThread(Thread):
    ...
    def run(self):
        ...
        while not self._stop_event.wait(DisplayStateThread.POLL_INTERVAL):
            if self._last_button and time.time() - self._last_button >= 60:
                self._last_button = None
                self._state = "idle"
                self._display.show_main_menu()
            if self._state == "sub_tank_week":
                self._state = "idle"
                self._display.show_sub_menu(1)
                times, values = self._settings.database.get_week("sensors.tank.value")
                self._display.show_plot_week(times, values)
                self._display.update()
            if self._state == "sub_tank_month":
                self._state = "idle"
                self._display.show_sub_menu(2)
                times, values = self._settings.database.get_week("sensors.tank.value")
                self._display.show_plot_month(times, values)
                self._display.update()
            if self._state == "sub_tank_year":
                self._state = "idle"
                self._display.show_sub_menu(2)
                times, values = self._settings.database.get_week("sensors.tank.value")
    ...
    def _on_sensor(self, _, old, new):
        ...
        if old != new:
            if self._state == "main":
                ...
                self._display.update_sensor_values()
    ...
    def _on_button(self, name, old, new):
        ...
        if self._state == "main":
            if button == 1:
                self._state = "sub_tank_week"
                self._settings.log.msg(
                    "DisplayStateThread: state sub_tank_week")
        elif self._state == "sub_tank":
            if button == 1:
                self._state = "main"
                self._settings.log.msg(
                    "DisplayStateThread: state main")
            elif button == 2:
                self._state = "sub_tank_week"
                self._settings.log.msg(
                    "DisplayStateThread: state sub_tank_week")
            elif button == 3:
                self._state = "sub_tank_month"
                self._settings.log.msg(
                    "DisplayStateThread: state sub_tank_month")
            elif button == 4:
                self._state = "sub_tank_year"
                self._settings.log.msg(
                    "DisplayStateThread: state sub_tank_year")
