...
class DisplayStateThread(Thread):
    ...
    def _on_button(self, name, old, new):
        ...
        if self._state == "main":
            if button == 1:
                self._state = "sub_tank"
                self._settings.log.msg(
                    "DisplayStateThread: state sub_tank")
            ...
            if button == 1:
                ...
                times, values = self._settings.database.get_month("sensors.tank.value")
                self._display.show_plot_week(times, values)
                self._display.update()
            elif button == 4:
                self._display.show_sub_menu(3)
                times, values = self._settings.database.get_year("sensors.tank.value")
                self._display.show_plot_week(times, values)
                self._display.update()
